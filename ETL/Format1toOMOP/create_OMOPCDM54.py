...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(logname, "w"), logging.StreamHandler()],
)
...
execute_query(
    create_schema.format(target_schema=target_schema),
    dbname,
    user,
    host,
    port,
...
query = read_ddl_sql_file(
    "OMOPCDM_postgresql_5.4_ddl.sql",
    folder_ddl,
    target_schema,
    replacement=replacements_format1,
)
execute_query(
...
query = read_ddl_sql_file(
    "OMOPCDM_postgresql_5.4_primary_keys.sql", folder_ddl, target_schema
)
execute_query(
    query.format(source_schema=source_schema, target_schema=target_schema),
    dbname,
...
logger.info(
    f"Total script execution time create OMOP and load vocabulary: {total_script_time:.4f} seconds"
)
