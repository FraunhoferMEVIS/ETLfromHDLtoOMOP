...
for table_ in tables:
    table = "V" + str(table_)
    table_occurrence = "V" + str(
        int(table_) + 1
    )  # for table sa151 sa751 sa999 and sa951 reported year = compensation year. For all other tables reported year = compensation year + 1.  Tables are named as following v{compensationyear}_sa{tablename}
    ...
    if not is_inserted:
        ...
        # table observation
        observation_sa151_sa152 = read_sql_file(
            "sa151sa152_to_observation.sql", folder_load
        )
        columns = [("SA151", "versichertentagekg")]
        for prefix_, column_ in columns:
            execute_query(
                observation_sa151_sa152.format(
                    source_schema=source_schema,
                    target_schema=target_schema,
                    table=table,
                    prefix=prefix_,
        ...
        columns = [
            ("sa152", "erwerbsminderungs_vt"),
            ("sa152", "versichertentageausland"),
            ("sa152", "versichertentage13ii"),
            ("sa152", "versichertentage53iv"),
        ]
        for prefix_, column_ in columns:
            execute_query(
                observation_sa151_sa152.format(
        ...
        execute_query(
            query_add_source_idx.format(target_schema=target_schema),
            dbname,
            user,
            host,
            port,
        ...
        # all other tables
        files = [
            "sa551_to_visit_occurrence.sql",
            "sa651_to_visit_occurrence.sql",
            "sa551_to_condition_occurrence.sql",
            "sa651_to_condition_occurrence.sql",
            "sa153_to_procedure_occurrence.sql",
            "sa651_to_observation.sql",
            "sa451_to_drug_exposure.sql",
        ]
        for file in files:
            query = read_sql_file(file, folder_load)
            execute_query(
        ...
        files = [
            "sa151_to_observation_period.sql",
            "sa151_to_death.sql",
            "sa151_sa951_to_payer_plan_period.sql",
        ]
        for file in files:
            query = read_sql_file(file, folder_load)
            execute_query(
        ...
        execute_query(
            query_drop_source_idx.format(target_schema=target_schema),
            dbname,
            user,
            host,
            port,
