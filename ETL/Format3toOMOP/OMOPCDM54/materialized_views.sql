...
SELECT
    replace(concept1.concept_code, '.', '') AS source_code,
...
FROM
    ...
    JOIN {target_schema}.domain domain ON concept2.domain_id::TEXT = domain.domain_id::TEXT
WHERE
    rel.invalid_reason IS NULL
    AND concept1.invalid_reason IS NULL
    AND concept1.vocabulary_id::TEXT = 'ICD10GM'::TEXT
